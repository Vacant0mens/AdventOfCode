...
wire1_arr = wire1.split(',')
wire2_arr = wire2.split(',')

def get_coordinates(line_arr):
  coordinates = []
  line_x = 0
  line_y = 0
  for i in  line_arr: 
    ...
    if direction == 'R':
        ...
        line_y -= num
    coordinates.append((line_x, line_y))
  return coordinates

coords_1 = get_coordinates(wire1_arr)
coords_2 = get_coordinates(wire2_arr)

pt1_answers = []
for coord_1 in coords_1:
    i = 0
    for coord_2 in coords_2:
        if i+1 >= len(coords_1) or i+1 >= len(coords_2):
            break
        if coord_2[0] <= coord_1[0] <= coords_2[i+1][0] and\
                coord_1[1] <= coord_2[1] <= coords_1[i+1][1]:
            cross = abs(coord_1[0]) + abs(coord_2[1])
            pt1_answers.append(cross)
        elif coord_1[0] <= coord_2[0] <= coords_1[i+1][0] and\
                coord_2[1] <= coord_1[1] <= coords_2[i+1][1]:
            cross = abs(coord_2[0]) + abs(coord_1[1])
            pt1_answers.append(cross)
        i += 1

pt1_answers.sort()
print(f"Part One Answer: {pt1_answers[0]}")